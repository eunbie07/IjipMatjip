...
def migrate_csv_to_db():
    ...
    try:
        ...
        for index, row in tqdm(df.iterrows(), total=len(df), desc="매물 데이터 처리 중"):
            deal_type, deposit, rent = parse_price(row['거래유형/가격'])
            if not deal_type or deal_type not in ['월세', '전세']: continue

            maintenance_fee = parse_maintenance_fee(row['관리비'])
            
            # --- 👇 '전용 면적'을 사용하도록 수정 ---
            area_m2_match = re.search(r'(\d+\.?\d*)㎡', str(row['전용 면적']))
            ...
            if lat and lng and area_m2:
                cur.execute(
                    ...
                    """,
                    (deal_type, deposit, rent, maintenance_fee, row['방 종류'], area_m2, 
                     row['층수'], row['상세주소'], row['준공년월'], row['방/욕실 수'],
                     photo_url, lat, lng, lng, lat)
                )
            time.sleep(0.05)
        
        conn.commit()
        print(f"🎉 새로운 매물 데이터가 성공적으로 DB에 저장되었습니다.")
